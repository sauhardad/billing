...
Class Report_model extends CI_Model
{
    ...
    function retrieveAllGroupReport($subsection_id)
    {
       ...
       /* $this->db->select('u.group_id as group_id,SUM(ROUND((tbl_teacher.share_percent/100)*u.amount))');
        $this->db->from('(SELECT `tbl_group`.`id` as group_id, `tbl_student_course`.`teacher_id`, `tbl_student_course`.`amount` FROM (`tbl_group`) LEFT JOIN `tbl_students` ON `tbl_students`.`group_id`=`tbl_group`.`id` INNER JOIN `tbl_student_course` ON `tbl_student_course`.`student_id`=`tbl_students`.`id` WHERE `tbl_group`.`subsection_id` = '.$subsection_id.') as u');
        $this->db->join('tbl_teacher','tbl_teacher.id=u.teacher_id','inner');
        $this->db->group_by("u.group_id");
        $this->db->order_by("u.group_id");
        $query=$this->db->get();
        ...
        //get total amount
        $this->db->select('tbl_group.id,tbl_group.name,SUM(tbl_student_course.amount) as amount,t.paid as paid');
